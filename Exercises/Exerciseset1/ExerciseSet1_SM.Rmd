...
1. 
<br>
The Second Column of Data Corresponds to ticker AAPL, i.e. Apple
The data is downloaded in dataSet1 line 8 where "AAPL" is specified in mytickers
<br>
<br>
2.
<br>
Apple Cumulative Return is `r round(100*sum(StockReturns[,2]),1)`%
<br>
Apple Average Daily Return is `r round(100*mean(StockReturns[,2]),3)`%
<br>
Apple standard deviation over the period is `r round(100*sd(StockReturns[,2]),3)`%
<br>
<br>
3. 
<br>
We can add the symbol in myticker to get the data. For example for Yahoo with the tickker symbol "YHOO" the data is shown below:
<br>
Yahoo Cumulative Return is `r round(100*sum(StockReturns[,3]),1)`%
<br>
Yahoo Average Daily Return is `r round(100*mean(StockReturns[,3]),3)`%
<br>
Yahoo standard deviation over the period is `r round(100*sd(StockReturns[,3]),3)`%
<br>
<br>
...
1.
<br>
We can use the nrow and ncol commands on transposedData.
<br>
<br>
<br>
2.
...
Transposedataportfolio = apply(transposedData[1:2,],2, mean)
names(Transposedataportfolio) <- colnames(transposedData)
pnl_plot(Transposedataportfolio)
```
<hr>
<hr>
...
<br>
1.
If we change the start date in dataset1 file then all the  data and graphs get modified.
<br>
2.
<br>
For new weighted portfolio add new companies by adding tickers in the dataset1.R
<br>
For plotting returns in a specific period  we create a sub set of the entire data and plot the same.
<hr>
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio3 = apply(StockReturns[2011:nrow(StockReturns),c(2:ncol(StockReturns))],1,mean)
pnl_plot(portfolio3)
```
<hr>
...
2.
<br>
The function performs matrix addition and if there is valid data in my data, it stores value in a defined matrix. If the output matrix is not defined or mydata is not valid matrix it gives an error.
<hr>
<hr>
